...
class OpenIDConnect {
  ...
  public function completeAuthorization(OpenIDConnectClientInterface $client, array $tokens, &$destination) {
    ...
    $settings = $this->configFactory->get('openid_connect.settings.okta')->get('settings');

    if ($settings['only_hcp'])
    {
        if (!isset($context['userinfo']['customer_id']) || 
            !isset($context['userinfo']['crm_country']) ||
            strlen($context['userinfo']['customer_id']) <= 10 ||
            strlen($context['userinfo']['crm_country']) != 2)
        {
          //$this->messenger->addError($this->t("You can't login to this site because you're not HCP", []));
          return FALSE;
        }
    }
