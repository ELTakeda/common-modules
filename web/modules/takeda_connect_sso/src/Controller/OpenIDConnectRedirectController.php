...
class OpenIDConnectRedirectController extends ControllerBase implements AccessInterface {
  ...
  public function access() {
    ...
    // If no state token - we assume this is redirect after logout
    if (!$state_token) {
      return AccessResult::allowed();
    }
  ...
  public function authenticate($client_name) {
    ...
    if (!$state_token)
    {
        ...
        $redirect = new RedirectResponse($host);
        return $redirect;
    }
    ...
    if ($request->get('error')) {
      ...
      $_SESSION['openid_connect']['tokens'] = $tokens;

      if ($tokens) {

          if (isset($_SESSION['test_only'])) {
              if ($_SESSION['test_only'] == true) {
        ...
        $this->messenger()->addError($this->t('Failed to get authentication tokens for @provider.', $provider_param));
      }
    }
