...
class OpenIDConnectSettingsForm extends ConfigFormBase implements ContainerInjectionInterface {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $form['other']['override_registration_settings'] = [
      '#type' => 'checkbox',
    ...
    $form['other']['always_save_userinfo'] = [
      '#type' => 'checkbox',
    ...
    foreach ($properties as $property_name => $property) {
      ...
      $form['other']['userinfo_mappings'][$property_name] = [
        '#type' => 'select',
    ...
    $form['other']['advanced']['connect_existing_users'] = [
      '#type' => 'checkbox',
  ...
  public function save_okta($form, $form_state) {

      $config = $form_state->getValue([ 'clients', 'okta', 'settings' ]);
      $this->configFactory()
        ...
        ->set('settings', array(
                   ...
                   'only_hcp' => $config['only_hcp']
              ))
        ->save();
  }

  public function test_okta($form, $form_state) {

    $this->session->saveDestination();
    $_SESSION['test_only'] = true;
    ...
    $response = $client->authorize($scopes, true);
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->config('openid_connect.settings')
      ->set('always_save_userinfo', $form_state->getValue([ 'other', 'always_save_userinfo' ]))
      ->set('connect_existing_users', $form_state->getValue([ 'other', 'advanced', 'connect_existing_users' ]))
      ->set('override_registration_settings', $form_state->getValue([ 'other', 'override_registration_settings' ]))
      ->set('userinfo_mappings', array_filter($form_state->getValue([ 'other', 'userinfo_mappings' ])))
      ->set('user_login_display', $form_state->getValue([ 'other', 'user_login_display' ]))
      ->save();
